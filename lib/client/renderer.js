...
function init (client, d3) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles ( ) {
    ...
    var loopSpam = ['DEBUG ', 'INTERNAL', 'sendGlucoseFutureLowNotifications:', 'EVENT: Status', 'EVENT: Settings', 'EVENT: Bolus'];
    ...
    var treatCircles = chart().focus.selectAll('treatment-dot').data(client.ddata.treatments.filter(function(treatment) {
      ...
      var notLoopSpam = enteredBy.indexOf('loop://') === -1 || _.isUndefined(_.find(loopSpam, function startsWith (spam) {

        return notes.indexOf(spam) === 0;
      }));

      return notCarbsOrInsulin && !treatment.duration && notTempOrProfile && notOpenAPSSpam && notLoopSpam;
    }));
  ...
  function prepareArc(treatment, radius) {
    ...
    /*
    if (treatment.foodType && treatment.foodType.length < 10) {
  ...
  renderer.drawTreatment = function drawTreatment(treatment, opts, carbratio) {
    ...
    var notes = treatment.notes || '';
    var enteredBy = treatment.enteredBy || '';
    if ( notes.startsWith('INTERNAL') || notes.startsWith('DEBUG') ) {
	    return;
    }
